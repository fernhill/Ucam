...
// var sockio_url = 'http://192.168.202.1:9090/'
var host = getUrl.host.split(":")[0]
var sockio_url = 'http://'+host+':9090/'
var HOST="http://"+host+":5000";
...
.factory('socket', ['$rootScope', function($rootScope) {
  var socket = io.connect(sockio_url);
 ...
 socket.on('reset_alert', function(data){
  ...
  socket.on('pos_data', function(data){
   ...
   socket.on("ETH_DOWN", function(data){
	 ...
	 $scope.$apply(function(){
		 ...
		 $scope.fault_img = "./images/red_l.png";
		 $scope.alarm_message = "Ethercat communication down. Please check connection and restart";
		 gmsg = "Ethercat communication down. Please check connection and restart";
	 });
     // alert("Ethercat Down");
  });
  ...
  socket.on("ETH_UP", function(data){
	 ...
	 $scope.$apply(function(){
		...
		// $scope.fault_img = "./images/green_l.png";
		 $scope.alarm_message = "Ethercat communication link is up..";
		 gmsg = "Ethercat communication link is up..";
	 })
  ...
  socket.on("alarms", function(message){
	...
	if(r[1][1] == 1){
		  $scope.almin = "./images/on.png";
	      $scope.fault_img = "./images/red_l.png";
		  $scope.alarm_message = "POT Hard Limit Switch Latched";
		  gmsg = "POT Hard Limit Switch Latched";
		  socket.emit('pot_hard_limit',{'status':'pot'});
		  
	  }
	  if(r[1][0] == 1){
		  $scope.almin = "./images/on.png";
	      $scope.fault_img = "./images/red_l.png";
		  $scope.alarm_message = "NOT Hard Limit Switch Latched";
		  gmsg = "NOT Hard Limit Switch Latched";
		  socket.emit('not_hard_limit',{'status':'not'});
	  }
...
.controller('HomeCtrl', function($scope, $http, $state,socket, $location, $ionicModal){
  ...
  socket.on('ref_complete', function(data){
    ...
    //Enable Button
    try{
      document.getElementsByClassName('zref_btn')[1].disabled = false;
    }catch(e){
    ...
    if($scope.mode == "step"){
      ...
      if($scope.display_text == "START JOG"){
        if($scope.p.feed_rate > 0 && $scope.p.feed_rate < 25){
		  ...
		  if(($scope.dir==0 && gmsg=="NOT Limit Exceeded") || ($scope.dir==1 && gmsg=="POT Limit Exceeded") || ($scope.dir==0 && gmsg=="NOT Hard Limit Switch Latched") || ($scope.dir==1 && gmsg=="POT Hard Limit Switch Latched")){
			//socket.emit("jog_mode",f);
...
.controller('SettingsCtrl', function($scope, $state) {
  ...
  $scope.get_access = function(passwd){
    ...
    if(passwd == "1986"){
      ...
      $scope.p.passwd = null;
	  $scope.err_msg = "Wrong Password. Please  try again."
    }
  }
  $scope.back = function(){
    $scope.passwd = "";
...
.controller('AutoCtrl', function($rootScope, $scope, $stateParams, $ionicModal, $state, socket, $http) {
  $scope.lineno = "";
  $scope.prev_line = -1;
...
.controller('SideMenuController', function($scope){
	console.log("Inside Side Menu Controller");
	$scope.show_manual_screen=true;
})
.controller('SettingsMenuCtrl', function($scope, $http, $state){
  ...
  $scope.goto_drive_offset = function(){
    $state.go("app.drive_offset");
  }
})

.controller('DriveOffsetCtrl', function($scope, $http, $state, socket, $ionicPopup){
	console.log("Inside Drive Offset Controller");
	$http.get(DAC_SETTINGS_API)
     .success(function(data){
       //console.log(data);
       $scope.params = {'X':{}}
       $scope.params.X = data.resp.X;
     })
     .error(function(err){
       //console.log(err);
     })
	 ...
	 $scope.udpate_settings = function(){
		 var keys = ['g54', 'g55', 'g56', 'g57', 'g58', 'g59'];
		 for(var i=0; i< keys.length; i++){
			 if($scope.params[keys[i]] === null || $scope.params[keys[i]] === undefined || $scope.params[keys[i]].length == 0){
				 $scope.params[keys[i]] = 0;
			 }
		 }

		 $http.post(DAC_SETTINGS_API, $scope.params)
	      .success(function(data){
...
.controller('DriveParamsCtrl', function($scope, $http, $state, socket, $ionicPopup){
  ...
  $scope.toggle = function(val){
    ...
    if(val == 'fin_signal'){
      $scope.tmp_fin_signal = $scope.params[val] == 1? '1' : '0';
    }
  }
  ...
  $http.get(DAC_SETTINGS_API)
   .success(function(data){
     ...
     if($scope.params.fin_signal == 1){
       $scope.tmp_fin_signal = "1";
     }else{
       $scope.tmp_fin_signal = "0";
     }
   })
   .error(function(err){
     //console.log(err);
